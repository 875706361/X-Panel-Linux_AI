	...
	for _, inbound := range inbounds {
		...
		if len(inbound.StreamSettings) > 0 {
			...
			if tlsSettings, ok := stream["tlsSettings"].(map[string]interface{}); ok {
				if panelSettings, ok := tlsSettings["settings"].(map[string]interface{}); ok {
			...
			if realitySettings, ok := stream["realitySettings"].(map[string]interface{}); ok {
				if panelSettings, ok := realitySettings["settings"].(map[string]interface{}); ok {
					for k, v := range panelSettings {
						if valStr, isStr := v.(string); isStr && valStr != "" {
							realitySettings[k] = v
						} else if !isStr && v != nil {
							realitySettings[k] = v
						}
				...
				// ---------------------------------------------------------
				// 【核心重构】: 如果解包后公钥仍为空，尝试从私钥推导 (Fail-Safe)
				// ---------------------------------------------------------
				if pub, _ := realitySettings["publicKey"].(string); pub == "" {
					if priv, _ := realitySettings["privateKey"].(string); priv != "" {
						if newPub := common.DeriveRealityPublicKey(priv); newPub != "" {
							realitySettings["publicKey"] = newPub
						}
					}
				}
			}
