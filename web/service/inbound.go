...
// deriveRealityPublicKey 现已移至 common.DeriveRealityPublicKey
...
func (s *InboundService) MigrationRequirements() {
	...
	logger.Info("开始执行数据库深度迁移与 REALITY 配置修复...")

	// 1. 同步流量汇总字段
	tx.Exec(`UPDATE inbounds SET all_time = IFNULL(up, 0) + IFNULL(down, 0) WHERE IFNULL(all_time, 0) = 0`)
	tx.Exec(`UPDATE client_traffics SET all_time = IFNULL(up, 0) + IFNULL(down, 0) WHERE IFNULL(all_time, 0) = 0`)
				...
				if clients, ok := settings["clients"].([]any); ok {
					for _, client := range clients {
						if c, ok := client.(map[string]any); ok {
							if _, has := c["speedLimit"]; !has {
								c["speedLimit"] = 0
								changed = true
							}
							if _, has := c["subId"]; !has {
								c["subId"] = common.GetRandomString(16)
								changed = true
							}
						}
					}
					if changed {
						newS, _ := json.Marshal(settings)
						ib.Settings = string(newS)
					}
				}
			}
		}

		// 2.2 修复 StreamSettings (REALITY)
			...
			if err := json.Unmarshal([]byte(ib.StreamSettings), &stream); err == nil {
				if security, _ := stream["security"].(string); security == "reality" {
					if rs, ok := stream["realitySettings"].(map[string]any); ok {
						rsSettings, _ := rs["settings"].(map[string]any)
						if rsSettings == nil {
							rsSettings = make(map[string]any)
							rs["settings"] = rsSettings
							changed = true
						}

						// 同步字段到 settings
						keys := []string{"publicKey", "fingerprint", "serverName", "spiderX", "mldsa65Verify"}
						for _, k := range keys {
							if v, exists := rs[k]; exists {
								rsSettings[k] = v
								delete(rs, k)
								changed = true
							}
						...
						// 公钥缺失修复
						pub, _ := rsSettings["publicKey"].(string)
						if pub == "" {
							priv, _ := rs["privateKey"].(string)
							if priv == "" {
								priv, _ = rsSettings["privateKey"].(string)
							}
							if priv != "" {
								if newPub := common.DeriveRealityPublicKey(priv); newPub != "" {
									logger.Infof("已修复入站 %d 的 REALITY 公钥", ib.Id)
									rsSettings["publicKey"] = newPub
									changed = true
								}
							}
						}
						...
						if changed {
							newST, _ := json.Marshal(stream)
							ib.StreamSettings = string(newST)
						}
					}
				}
			}
